...
| **Nginx Demo App** | `dev` | `http://localhost:30080` | N/A |
...
2. Deploy Argo CD and expose the web dashboard via NodePort.
   ...
   # 3. Verify all Argo CD pods are running
   kubectl get pods -n argocd

   # 4. Check the NodePort exposure
   kubectl get svc argocd-server -n argocd
   ```
   ![argo-pods](/assets/gitops/argocd-pods.jpg)

3. Log in to the Argo CD server and set up the credentials
   ```sh
   # Access ArgoCD through web browser
	http://localhost:30082

   # Retrieve Initial Admin Password (Username: admin)
...
1. Deploy Nginx demo application to ArgoCD
   ...
   ```
   ![nginx-pods](/assets/gitops/nginx-pods.jpg)

2. Access Argo CD at `http://localhost:30082` to verify the application deployment, sync, and health status.
   ![argocd-deploy](/assets/gitops/argocd-deploy.jpg)

3. Test Local Application Access
   ```sh
   # Access the Nginx demo application directly via the NodePort endpoint or through a web browser.
   curl -I http://localhost:30080
   ```
   ![app-verify](/assets/gitops/nginx-app.jpg)
